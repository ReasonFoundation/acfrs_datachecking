...
# problem: government_ID in pop_pop_governmentID has 14 characters (05201902700000) >< some government_ID in ACFRs has 13 characters 5201902700000
# look at those in acfrs
# Is this safe to assume that these government_ID in ACFRs data should have 0 at the beginning? 
...
# joining 2: including add 0 to those with 13 chars --> # once added extra 0, have more 695 matches 
...
# checking some cases: 
#Los Angeles in acfrs does not get matched with pop census
acfrs_city_pop %>% 
...
write.csv(acfrs_city_pop, "acfrs_city_pop.csv")
write.csv(acfrs_city_pop_added_char, "acfrs_city_pop_added_char.csv")

# for Jordan to map
acfrs_city_pop_formap <- acfrs_city_pop_added_char %>% 
  rename(entity = name) %>% 
  mutate(lia_rev_ratio = (round(total_liabilities/revenues, 4))*100) %>% 
  select(state.abb, state.name, geo_id, government_ID, entity, total_liabilities, revenues, lia_rev_ratio)

write.csv(acfrs_city_pop_formap, "acfrs_city_pop_formap.csv")
```
