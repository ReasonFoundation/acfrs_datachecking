...
county_pop_census_acfrs <- acfrs_county_parish %>% 
            left_join(pop) %>% 
            drop_na(population) %>% 
...
There are `r nrow(census_pop_NOT_match_acfrs_100k)` counties in Census that have more than 100k population that are not yet matche with ACFRs. 
...
There are `r nrow(census_pop_NOT_match_acfrs_all)` counties in Census, regarless of population size, that are not yet matche with ACFRs. 
